...
Bounce timeButton = Bounce(timeButtonPin, 20);
...
boolean timeButtonPressed = false;

boolean displayTimeSet =true;
...
void seTime () {
  ...
  timeButton.update();
  int timeControlValue = timeButton.read();

  if (timeControlValue == HIGH) {
  ...
  if ( rightValue == HIGH) {
    ...
    rtc.writeProtect(false);
    rtc.setTime(timeHour, timeMin, 0);
    delay(500);
...
void timesetCycle(){
  if(millis() - setSwapTime >= 500) {
    setSwapTime = millis();
    displayTimeSet = !displayTimeSet;
    Serial.print("Time Set cycle:");
    Serial.println(setSwapTime);
  }
}
...
void showSetTime() {
  int timeMin = EEPROM.read(4);
  int timeHour = EEPROM.read(3);
  displayTime(timeMin, timeHour);
}
